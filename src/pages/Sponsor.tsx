            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12">
              {goldSponsors.map((sponsor, index) => (
                ...
                >
                    ...
                    <div
                      className="mx-auto w-full max-w-xs rounded overflow-hidden"
                      style={{ display: 'block', aspectRatio: '9 / 16', position: 'relative' }}
                    >
                      <img
                      src={sponsor.image}
                      alt={sponsor.name}
                      ref={(img: HTMLImageElement | null) => {
                        ...
                        if (nw && nh) {
                          // se l'immagine è orizzontale (width > height) forziamo 9:16 (portrait),
                          // altrimenti 16:9 (landscape)
                          parent.style.aspectRatio = nw > nh ? '9 / 16' : nh > nw ? '16 / 9' : '1 / 1';
                        }
                        // Assicuriamoci che l'immagine riempia il contenitore mantenendo l'aspect ratio
                        img.style.width = '100%';
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-12">
              {sponsorshipPackages.map((pkg, index) => (
                <motion.div
                  ...
                  style={{ textAlign: 'center' }}
                >
                  <div className={`p-4 text-center text-white ${index === 0 ? 'bg-hyria-primary' :
